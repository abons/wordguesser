...
(() => {
  ...
  function woo(code, badge, label, minLen, maxLen, extra = "") {
    return { code, badge, label, url: `${MIRROR}/${code}-v1.txt`, extra, fold: true, minLen, maxLen };
  }
  var LANGUAGES = [
    {
      code: "en",
    ...
    { code: "fr", badge: "FR", label: "Fran\xE7ais", url: `${MIRROR}/fr-v1.txt`, extra: "", fold: true, minLen: 5, maxLen: 8 },
    { code: "de", badge: "DE", label: "Deutsch", url: `${MIRROR}/de-v1.txt`, extra: "\xDF", fold: true, minLen: 4, maxLen: 8 },
    { code: "es", badge: "ES", label: "Espa\xF1ol", url: `${MIRROR}/es-v1.txt`, extra: "", fold: true, minLen: 4, maxLen: 8 },
    woo("it", "IT", "Italiano", 5, 8),
    woo("pt", "PT", "Portugu\xEAs", 4, 8),
    woo("ca", "CA", "Catal\xE0", 4, 8),
    woo("ro", "RO", "Rom\xE2n\u0103", 4, 8),
    woo("sv", "SV", "Svenska", 4, 8),
    woo("cs", "CS", "\u010Ce\u0161tina", 4, 8),
    woo("sk", "SK", "Sloven\u010Dina", 4, 8),
    woo("da", "DA", "Dansk", 4, 8, "\xC6\xD8"),
    woo("nb", "NB", "Norsk", 4, 8, "\xC6\xD8"),
    woo("pl", "PL", "Polski", 4, 8, "\u0141"),
    woo("hr", "HR", "Hrvatski", 5, 8, "\u0110")
  ];
...
${URL2}`;
  ...
  function render(message = "") {
    ...
    app.append(board);
    const base2 = g.done ? g.won ? `Solved in ${g.guesses.length} \u2014 it was ${g.target}` : `Out of guesses \u2014 it was ${g.target}` : "";
    const text = g.done ? message ? `${base2} \xB7 ${message}` : base2 : message;
    app.append(el("div", "message", text));
    ...
    if (g.lang.extra.length > 0) {
      const r = el("div", "krow");
      for (const c of g.lang.extra) {
        const s = best.get(c);
        r.append(key(c, s === void 0 ? "" : stateClass(s)));
      }
      kb.append(r);
...
Solve the daily to earn a coin \u2014 once a day. Lose a game with a streak of ${SAVE_THRESHOLD}+ and you can spend a coin to keep the streak alive instead of it resetting to zero.`
  ...
  function wireControls() {
    ...
    const rebuildLenOptions = () => {
      const lang = languageByCode(langSel.value);
      const keep = Number(lenSel.value);
      lenSel.textContent = "";
      for (let len = lang.minLen; len <= lang.maxLen; len++) {
        const o = document.createElement("option");
        o.value = String(len);
        o.textContent = String(len);
        if (len === (keep >= lang.minLen && keep <= lang.maxLen ? keep : 5)) o.selected = true;
        lenSel.append(o);
      }
    };
    langSel.addEventListener("change", rebuildLenOptions);
    rebuildLenOptions();
    ...
    document.addEventListener("keydown", (e) => {
      ...
      const u = uc.length === 1 ? uc : e.key;
      if (u >= "A" && u <= "Z" || (game?.lang.extra.includes(u) ?? false)) onKey(u);
    });
  }
